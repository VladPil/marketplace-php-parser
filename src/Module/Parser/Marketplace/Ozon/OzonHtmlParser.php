...
final class OzonHtmlParser
{
     ...
     *
     * Ozon НЕ использует Schema.org JSON-LD. Данные извлекаются из:
     * - <h1> для названия товара
     * - HTML-encoded JSON с cardPrice/price/originalPrice
     * - Паттерн "X.X · N отзывов" для рейтинга
     * - JSON-поле "brand":"..." для бренда
     *
    ...
    public function parseProductData(string $html): ?array
    {
        ...
        if (preg_match('/<h1[^>]*>(.*?)<\/h1>/si', $html, $match)) {
            $result['name'] = trim(strip_tags($match[1]));
            $hasData = true;
        }
        ...
        if (preg_match('/(\d\.\d)\s*[·•]\s*([\d\s]+)\s*отзыв/u', $html, $match)) {
        ...
        // Бренд
        if (preg_match('/"brand"\s*:\s*"([^"]+)"/', $html, $match)) {
            $result['brand'] = $match[1];
            $hasData = true;
        }

        // Основное изображение — первый multimedia в wc1000
        if (preg_match('#https?://ir\.ozone\.ru/s3/multimedia-\d+-\w+/wc1000/\d+\.\w+#i', $html, $match)) {
