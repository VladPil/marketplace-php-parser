...
final class SessionManager implements SessionManagerInterface
{
    ...
    private function solveWithRetry(string $proxy, ?string $taskId = null): ?SessionData
    {
        ...
        // Записываем failure в health tracker — solver не смог получить cookies через эту прокси
        // Для rotating прокси пропускаем — health/CB для них бессмысленны
        if ($proxy !== 'direct' && !$this->isRotatingProxy($proxy)) {
            $proxyId = md5($proxy);
            $this->proxyRotator->recordFailure($proxyId);
            $this->logger->warning(
                sprintf('[proxy] Solver не смог получить cookies через прокси %s — записан failure', $this->maskProxy($proxy)),
                ['proxy_id' => $proxyId, 'channel' => 'proxy'],
            );
        }
    ...
    /**
     * Проверяет, является ли прокси ротационным.
     */
    private function isRotatingProxy(string $address): bool
    {
        return $this->proxyProvider !== null && $this->proxyProvider->getTypeByAddress($address) === 'rotating';
    }
