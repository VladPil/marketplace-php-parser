...
use App\Module\Parser\Identity\IdentitySanitizer;
use App\Module\Parser\Identity\IdentityWarmer;
...
final class ParseRunCommand extends Command
{
    public function __construct(
        ...
        private readonly IdentityWarmer $identityWarmer,
        private readonly IdentitySanitizer $identitySanitizer,
    ) {
        parent::__construct();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        Coroutine::set(['socket_timeout' => 90]);
        Coroutine\run(function () use ($output): void {
            ...
            Coroutine::create(function (): void {
                echo "[Identity Warmer] Запуск\n";
                $this->identityWarmer->run();
            });

            Coroutine::create(function (): void {
                echo "[Identity Sanitizer] Запуск\n";
                $this->identitySanitizer->run();
            });
            ...
            $output->writeln(sprintf(
                'Парсер запущен: %d воркеров + warmer + sanitizer, порт %d',
                $this->config->workers,
