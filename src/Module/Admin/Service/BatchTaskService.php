...
final class BatchTaskService
{
     ...
     *
     * Сначала сохраняет сущности в БД (батчами по BATCH_FLUSH_SIZE),
     * затем публикует в Redis — чтобы парсер не получил задачу раньше,
     * чем она появится в parse_tasks (FK-constraint на solver_sessions и др.).
     *
    ...
    public function createTasksFromFile(UploadedFile $file, string $type, string $marketplace, bool $collectProductData = false): array
    {
        ...
        $skipped = 0;
        $errors = [];
        /** @var array<array{id: string, type: string, params: array, marketplace: string}> $pendingMessages */
        $pendingMessages = [];
        ...
        foreach ($result['items'] as $index => $item) {
            ...
            if (($index + 1) % self::BATCH_FLUSH_SIZE === 0) {
                ...
                // Публикуем в Redis только ПОСЛЕ записи в БД
                foreach ($pendingMessages as $msg) {
                    $this->queueService->publishTask($msg);
                }
                $pendingMessages = [];
            }
        }
        ...
        if ($created > 0) {
            ...
            // Публикуем оставшиеся сообщения после финального flush
            foreach ($pendingMessages as $msg) {
                $this->queueService->publishTask($msg);
            }
        }
